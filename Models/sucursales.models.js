...
class Sucursales {
    ...
    static searchSucursal(id_Usuario, Direccion){
        const query = "SELECT * FROM Sucursales, SucursalesTieneUsuarios WHERE Sucursales.id_Sucursal = SucursalesTieneUsuarios.id_Sucursal AND id_Usuario = ? AND Direccion = ?";
        return db.execute(query, [id_Usuario, Direccion]);
    }
}
